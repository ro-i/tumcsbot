...
class CommandParserTest(unittest.TestCase):
    ...
    def _do_parse_args(self, command: str | None) -> dict[str, Any] | None:
    ...
    def _do_parse_opts(self, command: str | None) -> dict[str, Any] | None:
        result: tuple[dict[str, Any], dict[str, Any]] | None
        result = self._do_parse(command)
        if result is not None:
...
class CommandParserTestArgs(CommandParserTest):
    ...
    def test_valid_simple(self) -> None:
        ...
        self.assertEqual(
            self._do_parse_args("test abc def"), {"arg1": "abc", "arg2": "def"}
        )
    ...
    def test_valid_greedy_int(self) -> None:
        ...
        self.assertEqual(
            self._do_parse_args("test abc 1 2 3"), {"arg1": "abc", "arg2": [1, 2, 3]}
        )
...
class CommandParserTestOpts(CommandParserTest):
    ...
    def test_valid_flags_opts_combined(self) -> None:
        ...
        self.assertEqual(
            self._do_parse_opts("test"), {"a": False, "b": None, "c": False}
        )
        self.assertEqual(
            self._do_parse_opts("test -bu"), {"a": False, "b": "u", "c": False}
        )
        self.assertEqual(
            self._do_parse_opts("test -bu -a"), {"a": True, "b": "u", "c": False}
        )
        self.assertEqual(
            self._do_parse_opts("test -c -bu -a"), {"a": True, "b": "u", "c": True}
        )
...
class CommandParserTestOptsArgsCombined(CommandParserTest):
    def test_valid(self) -> None:
        ...
        self.assertEqual(
            self._do_parse("test -a 1 a b c"),
            ({"a": True}, {"arg1": 1, "arg2": ["a", "b", "c"]}),
        )
        self.parser.add_subcommand(
            "test2",
            opts={"i": int, "a": None},
    ...
    def test_valid_end_of_options(self) -> None:
        ...
        self.assertEqual(
            self._do_parse(r"test -a \\-b"),
            ({"a": True, "b": None}, {"arg1": "-b", "arg2": None}),
        )
        self.assertEqual(
            self._do_parse(r"test -a \\-b c"),
            ({"a": True, "b": None}, {"arg1": "-b", "arg2": "c"}),
        )
